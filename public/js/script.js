// i stayed up late to make this work and the later i stayed up the lower the
...
fetch('/api/pokemon')
    ...
    .then(data => {
      ...
      if (!sessionStorage.getItem('smashOrPass'))
        sessionStorage.setItem('smashOrPass', '[]');
      if (!sessionStorage.getItem('smashCount'))
        sessionStorage.setItem('smashCount', 0);
      if (!sessionStorage.getItem('passCount'))
        sessionStorage.setItem('passCount', 0);
      sessionStorage.setItem('pokemonLength', data.length);
    })
    .then(() => {
      const pokemon = JSON.parse(sessionStorage.getItem('pokemon'));

      fetch(`https://pokeapi.co/api/v2/pokemon/${currentPokemon ?? pokemon[0]}`)
          .then(res => res.json())
          .then(res => {
      ...
      if (sessionStorage.getItem('smashOrPass').length >= 0) {
        ...
        smashOrPass.forEach(item => {
          ...
          item.type === 'smash'
              ? smashPokemon.innerHTML += `<em><p>${item.name}</p></em>`
              : passPokemon.innerHTML += `<em><p>${item.name}</p></em>`;
        })
      }
      // Get the smash count
      ...
      if (sessionStorage.getItem('smashCount') +
              sessionStorage.getItem('passCount') >
          0) {
        const smashCount = JSON.parse(sessionStorage.getItem('smashCount'));
        const passCount = JSON.parse(sessionStorage.getItem('passCount'));

        smashPercentage.innerHTML =
...
smash.addEventListener('click', (e) => {
  ...
  if (e.isTrusted) {
    // push to array
    smashOrPass.push(JSON.stringify({
      type : "smash",
      name : sessionStorage.getItem('currentPokemon') ?? 'bulbasaur'
    }));
    // add to counter
    smashCount++;
    // add to list
    smashPokemon.innerHTML += `<em><p>${
        sessionStorage.getItem('currentPokemon') ?? 'bulbasaur'}</p></em>`;
    // add to displayed number
    smashNum.innerHTML = "Total: " + smashCount;

    // Display the percentage
    smashPercentage.innerHTML =
        Math.round((smashCount / (smashCount + passCount)) * 100) + "% Smash";

    // if current pokemon is the same or doesn't exist
    if (currentPokemon === pokemon[0] || currentPokemon === null)
      pokemon.shift();
    ...
    // fetch next pokemon
    fetch(`https://pokeapi.co/api/v2/pokemon/${pokemon.shift()}`)
        .then(res => res.json())
        .then(res => {
          sessionStorage.setItem('pokemon', JSON.stringify(pokemon));
          pokemonImage.src =
